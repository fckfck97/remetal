...
class Home(LoginRequiredMixin,TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        gastos = ComprasEnc.objects.filter(fecha_compra__year=ano,fecha_compra__month=mes).aggregate(Sum('gastos_adicionales'))
        descuento = FacturaEnc.objects.filter(fecha__year=ano,fecha__month=mes).aggregate(Sum('descuento'))
        #ganancias por mes
        compra_ganancia_mes = ComprasEnc.objects.filter(fecha_compra__year=ano,fecha_compra__month=mes).aggregate(Sum('total'))
        venta_ganancia_mes = FacturaEnc.objects.filter(fecha__year=ano,fecha__month=mes).aggregate(Sum('total'))
        ...
        total =  venta_ganancia_mes['total__sum'] - compra_ganancia_mes['total__sum'] 
        if total < 0 :
            total = total * -1
        #datos anual
        compra_ganancia_ano = ComprasEnc.objects.filter(fecha_compra__year=ano).aggregate(Sum('total'))
        venta_ganancia_ano = FacturaEnc.objects.filter(fecha__year=ano).aggregate(Sum('total'))
        ...
        facturacion_total = []
        for n in range(1,13):
            compra = ComprasEnc.objects.filter(fecha_compra__year=ano,fecha_compra__month=n).aggregate(Sum('total'))
            factura = FacturaEnc.objects.filter(fecha__year=ano,fecha__month=n).aggregate(Sum('total'))
